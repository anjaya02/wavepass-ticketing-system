  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.data.token) {
        ...
        // Decode the token to extract vendor ID
        const decoded: TokenPayload = jwtDecode(response.data.token);
        const vendorId = decoded.id; 

        // Update authentication context
        login(response.data.token, "vendor", vendorId);

        // Redirect to the vendor dashboard with replace option
        navigate("/vendor/ticket-management", { replace: true });
      } else {
